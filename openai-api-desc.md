...
curl -H "Content-Type: application/json" -H "Authorization: Bearer 你在ztoken的key" -XPOST https://api.ztoken.pro/v1/chat/completions -d '{"messages": [{"role":"user","content":"请介绍一下你自己"}]}'  | iconv -f utf-8 -t utf-8  
2、messages传的不对，messages是array
3、api_secret_key传的不对，不能再传openai的key了，你要传你从ztoken拿到的key（不需要有openai的key）
4、如果是python，注意openai包的版本要对，它升级了！！要注意，关键是base_url要设置成ztoken的，如果这个不正确，其它肯定都不行。所以一定要注意他在不同的包中base_url的设置方式，目前已知的是：在老版本中的设置方式是：openai.api_base = BASE_URL，而在新版本中的设置方式是：client = OpenAI(api_key=API_SECRET_KEY, base_url=BASE_URL)，别问为什么，问就是openai的锅      
```
注：<br>
1、以下所有接口的base_url: `https://api.ztoken.pro` <br>
2、API通过HTTP请求调用。每次请求，需要在HTTP头中携带用户的api_secret_key，用于认证。 开发者单独的api_secret_key，请从ztoken管理后台获得。 
请求头形如：  
```
Content-Type: application/json
Authorization: Bearer 你在ztoken的key  
```

（更详细API文档地址：http://doc.ztoken.pro）
#### 示例接口1、创建chat
...
| :-------- | :--------| :------ |
| Content-Type| application/json| | 
| Authorization| Bearer 你在ztoken的key| 开发者单独的api_secret_key，请从ztoken管理后台获得。 | 

- **请求参数**
>
| 请求参数      |     参数类型 |   是否必须   |参数说明   |
| :-------- | :--------| :------ | :------ |
| model| string| 是| 大模型的类别，包括但不限于：gpt-4-1106-preview，gpt-4-vision-preview，gpt-3.5-turbo-1106，gpt-3.5-turbo-instruct，gpt-4, gpt-4-0314,  gpt-4-0613, gpt-4-32k, gpt-4-32k-0613, gpt-3.5-turbo, gpt-3.5-turbo-0613, gpt-3.5-turbo-16k, gpt-3.5-turbo-16k-0613，还有claude，gemini等，还有众多国内模型。|
...
curl -H "Content-Type: application/json" 
     ...
     -XPOST https://api.ztoken.pro/v1/chat/completions -d '{
  "messages": [
    {"role":"user","content":"请介绍一下你自己"},
    {"role":"assistant","content":"您好，我是ztoken机器人。我能够与人对话互动，回答问题，协助创作，高效便捷地帮助人们获取信息、知识和灵感。"},
...
// 调用请求
$cburl = 'https://api.ztoken.pro/v1/chat/completions';
...
openai.base_url = "https://api.ztoken.pro/v1"  # 要注意openai的版本号，目前最新的是base_url，旧版可能是api_base
...
def chat_completions():
    url="https://api.ztoken.pro/v1/chat/completions"
